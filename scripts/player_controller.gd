...
@export var score_component: Score

@export var vulnerability_component: Vulnerability

@export var stabilizer_component: StabilizarShootComponent
...
@onready var bullet_scene: PackedScene = preload("res://scenes/bullet.tscn") as PackedScene
...
func _physics_process(delta):
	...
	position += velocity
	position.y = max(
		sprite_component.get_rect().size.y / 2,
		min(position.y, limit_y - sprite_component.get_rect().size.y / 2)
	)
	position.x = max(
		sprite_component.get_rect().size.x / 2,
		min(position.x, limit_x - sprite_component.get_rect().size.x / 2)
	)
